...
x <- enrichPathway(gene=ids$ENTREZID, pvalueCutoff = 0.05, readable=TRUE)
viewPathway("Platelet activation, signaling and aggregation", 
            readable = TRUE)
```
...
p3 <- barplot(ans.do, showCategory=10) + ggtitle('Never vs Current Smokers') + theme(plot.title = element_text(size = 25))
p3
ans.go <- pairwise_termsim(ans.go)
